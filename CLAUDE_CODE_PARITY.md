# Noryx and Claude Code — evidence boundary

## Version 3.8.4 competitive contract

The product goal remains to outperform Claude Code. Version 3.8.4 retains the 3.8.0 competitive gate and converts that goal into a fail-closed qualification contract rather than lowering it or declaring success from internal smoke tests.

Noryx now requires a sealed three-way campaign:

1. Noryx with the selected underlying model.
...
The private evaluator key must never be stored in the Noryx repository or its
product CI. The public key and sealed campaign identity used for authorization
must arrive through the separate operator trust policy; values embedded in the
report are diagnostic only and cannot authorize that report.


# Noryx local-agent parity audit
...
“Parity” here means local agentic coding capability, not a claim that Noryx can reproduce Anthropic-account services. Cloud hosting, Claude subscriptions, proprietary models, phone push delivery, claude.ai Artifacts, Remote Control, Chrome computer use, Slack/GitHub hosted apps, organization analytics, and Anthropic-managed Routines require external services Noryx does not own. They are explicitly out of scope rather than simulated.

## Implemented and enforced

| Claude Code capability | Noryx implementation |
|---|---|
...
| Project instructions/memory | Combined nearest `NORYX.md`, `AGENTS.md`, trusted `CLAUDE.md`, and project decision memory |
...
| Background work | Start, poll, read complete logs, and stop Noryx-owned typed processes |
...
These are not part of the Noryx 3.2.1 open CLI specification. Host-dependent
...
Noryx never converts an unavailable check into verified success.
