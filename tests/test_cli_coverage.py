        ...
        with patch("nexus.cli.cli_impl.Agent") as MockAgent:
            with patch.object(sys, "argv", ["nexus", "!echo hello"]):
                ...
                mock_agent._execute_tool_with_safety.assert_called_once_with(
                    "run_process",
                    {"argv": ["echo", "hello"], "cwd": mock_agent.working_dir},
    ...
    with patch.dict(os.environ, {"NVIDIA_API_KEY": "test"}):
        with patch("nexus.cli.cli_impl.Agent") as MockAgent:
            with patch.object(sys, "argv", ["nexus", "--confirm-danger", "!rm -rf ./sentinel"]):
                ...
                mock_agent._execute_tool_with_safety.assert_called_once_with(
                    "run_process",
                    {"argv": ["rm", "-rf", "./sentinel"], "cwd": mock_agent.working_dir},
                    _user_initiated=True,
    ...
    with patch.dict(os.environ, {"NVIDIA_API_KEY": "test"}):
        with patch("nexus.cli.cli_impl.Agent") as MockAgent:
            with patch.object(sys, "argv", ["nexus", "!rm -rf /"]):
                mock_agent = MockAgent.return_value
                mock_agent._execute_tool_with_safety.return_value = (
                    "⏸️ PENDING_CONFIRMATION [danger-0001]: ...",
                    False,
                )
                try:
                    main()
                ...
                assert (
                    "Run with --confirm-danger to execute this exact command: !rm -rf /"
                    in captured.out
                )
...
def test_handle_run_management_all():
    ...
    with patch.object(sys, "argv", ["nexus", "runs", "--json"]):
        with patch("nexus.cli.cli_impl.RunCatalog") as MockCatalog:
            ...
            MockCatalog.return_value.list.return_value = [
                SimpleNamespace(session_id="a", turn_id="b", status="c", request="d")
            ]
    ...
    with patch.object(sys, "argv", ["nexus", "benchmark", "--manifest", "nonexistent.json"]):
        with patch("nexus.benchmark.BenchmarkSuite.load"):
            with patch("nexus.benchmark.BenchmarkRunner") as MockRunner:
                ...
                MockRunner.return_value.run.return_value = SimpleNamespace(
                    to_dict=lambda: {"summary": {"failed": 0, "tasks": 1}}
                )
...
def test_handle_generate_dashboard_valid():
    ...
    with patch.object(
        sys, "argv", ["nexus", "generate-dashboard", "--input", "in.json", "--output", "out.html"]
    ):
...
def test_handle_slash_commands_all():
    ...
    with (
        patch("nexus.cli.cli_impl.Agent") as MockAgent,
        patch("nexus.cli.cli_impl.get_history") as mock_get_history,
        patch("nexus.cli.cli_impl.ConversationMemory") as mock_mem,
        patch("nexus.cli.cli_impl.start_background_web_server"),
    ):
        agent = MockAgent.return_value
        agent.model_key = "test"
        agent.model_cfg = {
        ...
        mock_mem.return_value.list_conversations.return_value = [
            {"id": "c1", "model_name": "x", "message_count": 1, "preview": "p"}
        ]
        assert handle_slash_command("/history", agent) is True
        ...
        agent.project_mem.load_rules.return_value = SimpleNamespace(
            build_command="b",
            test_command="t",
            lint_command="l",
            format_command="f",
        ...
        with patch("nexus.github.GitHubIntegration.view_pr") as mock_view_pr:
            ...
            mock_view_pr.return_value = {
                "number": 1,
                "title": "test",
                "comments": [{"author": {"login": "testuser"}, "body": "test comment"}],
            }
            assert handle_slash_command("/pr_comments 1", agent) is True
