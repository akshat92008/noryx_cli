...
def test_sandbox_credentials_leak_prevention(tmp_path, monkeypatch):
    sandbox = SandboxRunner(workspace=str(tmp_path))

    # Mock environment
    monkeypatch.setenv("NEXUS_API_KEY", "secret123")
    monkeypatch.setenv("NEXUS_TOKEN", "token123")
    monkeypatch.setenv("NEXUS_SECRET_SAUCE", "sauce123")
    monkeypatch.setenv("NEXUS_SAFE_VAR", "safe123")
    monkeypatch.setenv("PATH", "/bin:/usr/bin")
    monkeypatch.setenv("PYTHONPATH", "/usr/lib/python")
...
def test_quality_mode_distinct_reviewer():
    ...
    agent.evidence.append(
        kind="file_mutation", claim="did something", status="verified", raw_output=""
    )
    ...
    assert (
        "Independent review failed closed: quality mode requires a reviewer model different from the executor."
        in message
    )
