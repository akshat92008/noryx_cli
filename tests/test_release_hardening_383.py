"""Regression coverage for Noryx CLI 3.8.3 autonomous hardening."""
...
def test_macos_prepare_redirects_temp_environment_into_workspace(tmp_path: Path, monkeypatch):
    runner = SandboxRunner(tmp_path)
    monkeypatch.setattr(runner, "backend", lambda: SandboxBackend.MACOS)
    prepared = runner.prepare(
        CommandSpec.create([sys.executable, "-V"], tmp_path, require_os_isolation=True)
    )
    try:
        expected = tmp_path.resolve() / ".noryx" / "sandbox-tmp"
        ...
        if prepared.cleanup_path:
            Path(prepared.cleanup_path).unlink(missing_ok=True)


def test_optional_process_isolation_really_allows_fallback(tmp_path: Path):
    spec = ProcessExecutionGateway._build_sandbox_spec(
        ProcessRequest.create("test", [sys.executable, "-V"], tmp_path, isolation_policy="optional")
    )
...
def test_required_process_isolation_still_fails_closed(tmp_path: Path):
    spec = ProcessExecutionGateway._build_sandbox_spec(
        ProcessRequest.create("test", [sys.executable, "-V"], tmp_path, isolation_policy="required")
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["git", "log", "-1"], CommandRisk.READ_ONLY),
        (["npm", "test"], CommandRisk.VALIDATION),
        (["go", "test", "./..."], CommandRisk.VALIDATION),
        (["npm", "run", "build"], CommandRisk.BUILD),
        (["pip", "install", "rich"], CommandRisk.PACKAGE_INSTALL),
        ([sys.executable, "-m", "pip", "install", "rich"], CommandRisk.PACKAGE_INSTALL),
        (["git", "commit", "-m", "x"], CommandRisk.GIT_MUTATION),
        (["git", "push"], CommandRisk.NETWORK_REQUEST),
    ],
)
...
def test_lsp_network_is_denied_by_default_and_explicitly_opt_in(tmp_path: Path, monkeypatch):
    ...
    class StopStart(RuntimeError):
        pass

    def fake_popen(request, **_kwargs):
        captured.append(request.network_policy)
        raise StopStart

    monkeypatch.setattr(ProcessExecutionGateway, "popen", fake_popen)
    with pytest.raises(StopStart):
        LSPClient(tmp_path, "python", command=(sys.executable, "-V")).start()
    with pytest.raises(StopStart):
        LSPClient(tmp_path, "python", command=(sys.executable, "-V"), network=True).start()
    assert captured == ["deny", "allow"]


def test_autonomous_admission_fails_closed_on_failed_behavioral_qualification(
    ...
    q = SimpleNamespace(
        ...
        backend="unisolated-host-process",
        probes=[SimpleNamespace(name="outside_write_denied", passed=False)],
    )
    monkeypatch.setattr(
        "nexus.platform.sandbox_qualification.qualify_native_sandbox", lambda _root: q
    )
    with pytest.raises(RuntimeError, match="Autonomous mode blocked"):
        cli_impl._require_autonomous_host_qualification(tmp_path)
...
@pytest.mark.skipif(
    os.name != "posix", reason="process-group hard ceiling regression is POSIX-specific"
)
def test_background_output_is_bounded_during_execution(tmp_path: Path, monkeypatch):
    ...
    while time.monotonic() < deadline:
        ...
        if "terminated after exceeding output ceiling" in status or "exited (" in status:
            break
        time.sleep(0.05)
    from nexus.tools.background import _bg_processes

    record = _bg_processes[pid]
    assert Path(record["stdout_log"]).stat().st_size <= ceiling
    assert record["output_limit_exceeded"] is True
    assert "output truncated by Noryx policy" in tool_process_status(pid)
    tool_process_stop(pid)


def test_release_binding_accepts_autonomy_only_with_sandbox_and_competitive_evidence(
    tmp_path: Path,
):
    import hashlib
    import json
