...
def test_integration_coordinator_rejects_missing_verifier():
    ...
    assert not any("STUB_PASS" in res for res in result.verification_results), (
        "STUB_PASS was found in verification results"
    )
    # Should contain an error or unavailable state
    assert any("VERIFICATION_UNAVAILABLE" in res for res in result.verification_results) or any(
        "ERROR" in res for res in result.verification_results
    ), "Did not fail closed on missing verifier"
...
def test_lead_orchestrator_fails_on_stub_pass(tmp_path: Path):
    ...
    async def run_test():
        ...
        orchestrator._reviewer.review = MagicMock(
            return_value=WorkerReview(
                assignment_id="test-assign-1",
                accepted=True,
                findings=(),
                missing_evidence=(),
        ...
        assert final_state.state != CollaborationState.COMPLETED, (
            "Orchestrator allowed a fake success to reach COMPLETED status"
        )
        assert final_state.state == CollaborationState.FAILED, (
            "Orchestrator should fail when verification is missing/invalid"
        )
