...
def main() -> int:
    ...
    for index in range(args.start, args.end):
        ...
        file_env = dict(env)
        file_env["COVERAGE_FILE"] = str(ROOT / f".coverage.release-{index + 1:03d}")
        command = [
            sys.executable,
            "-m",
            "coverage",
        ...
        result = subprocess.run(
            command, cwd=ROOT, env=file_env, text=True, capture_output=True, timeout=180
        )
        ...
        print(
            f"PASS {index + 1:03d}/{len(files)} {test_file.name} ({elapsed:.2f}s) {last}",
            flush=True,
        )
    return 0
