...
def main():
    ...
    if is_windows:
        if "No supported OS sandbox is available" not in data.get("result", ""):
            print(
                "ERROR: Windows should fail closed due to lack of sandbox backend.", file=sys.stderr
            )
    ...
    if code == 0 or data.get("success"):
        print(
            "ERROR: Dangerous command executed successfully without confirmation.", file=sys.stderr
        )
        return 1
    ...
    if code == 0 or data.get("success"):
        print(
            "ERROR: Package installation succeeded but should have been blocked.", file=sys.stderr
        )
        return 1
    ...
    if "PENDING_CONFIRMATION" not in result_text and "BLOCKED" not in result_text:
        print(
            "ERROR: Package installation did not return PENDING_CONFIRMATION or BLOCKED.",
            file=sys.stderr,
        )
        print(data, file=sys.stderr)
        return 1
