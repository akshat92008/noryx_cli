...
Noryx is an open-source, multi-provider coding CLI focused on verification, worktree safety, and extensibility. A hosted model plans and reviews difficult work; local Nova V11 executes suitable atomic changes through Ollama. Noryx owns repository understanding, permissions, workspaces, tools, tests, repair, evidence, budgets, rollback, and recovery.

Version 3.8.5-rc3 is a release-candidate hardening build focused on request semantics, deterministic read-only execution, mutation integrity, policy fail-closed behavior, browser value extraction, and release-source consistency. The semantic compiler separates action, response mode, and side-effect authority before execution; ambiguous requests default to read-only rather than mutation. Native command isolation remains fail-closed by default, while trusted-host execution is available only through an explicit capability and the active permission-mode contract. A strict three-way superiority gate compares Noryx, the same underlying model without Noryx, and real Claude Code on private unseen repositories; no smoke or self-authored result can authorize a competitive claim.
...
- the matched benchmark gate compares the identical task, model, content-hashed repository revision, and sealed budget directly versus Noryx and measures verified uplift, false completion, regressions, budget compliance, cost, tokens, latency, and intervention;
...
Noryx 3.8.5-rc3 is a **beta release candidate for repository analysis, planning, and supervised Verified Repair**. The exact source now collects its full test inventory and the high-risk launch, semantic, repository-intelligence, mutation, tool, browser, and policy regression suites are expected to pass before promotion. Mandatory human diff review remains the deployment baseline. Autonomous generated-command execution stays fail-closed unless the active mode explicitly permits trusted-host execution or the exact operation is confirmed with the explicit host capability. Noryx does not claim universal Claude Code parity or unattended production autonomy; those claims remain gated by real comparative runs on private unseen repositories.
...
```bash
pipx install git+https://github.com/akshat92008/noryx_cli.git
noryx --version
noryx --doctor
```
...
```bash
git clone https://github.com/akshat92008/noryx_cli.git
cd noryx_cli
python3 -m venv .venv
.venv/bin/pip install .
.venv/bin/noryx --doctor
```

The distribution name is `noryx-cli`; the installed command is `noryx`.
The similarly named `nexusai` project on PyPI is unrelated.

Optional deterministic adapters:

```bash
pip install "noryx-cli[intelligence]"  # Tree-sitter language pack
pip install "noryx-cli[browser]"       # Playwright API
...
Noryx also reads `.env` from the current project, the Noryx checkout,
`~/.config/noryx/.env`, or `~/.noryx/.env`. An existing process environment
value always wins. Avoid passing secrets on the command line.

Set `NORYX_DISABLE_NETWORK=1` to fail closed before hosted-provider, web-fetch,
...
The model artifact is intentionally not bundled in the Python wheel. Noryx
...
# Flagship bounded repair with a hard rupee budget and Noryx Proof receipt
noryx fix \
...
noryx run --prompt "add a health endpoint and tests" --mode autonomous --output json

# Local-only Nova v11
noryx --model nova_codex

# Hosted planner with Nova available for atomic subtasks
noryx --model glm-5.2

# Read-only planning
noryx run --prompt "plan the authentication migration" --mode plan

# Review-only and autonomous execution
noryx run --prompt "implement the feature and test it" --mode review
noryx run --prompt "implement the feature and test it" --mode autonomous

# Hard logical-call, physical-attempt, token, and configured-cost limits
noryx --max-hosted-calls 8 --max-provider-attempts 12 \
...
noryx run --prompt "add regression tests" --local-only
noryx run --prompt "fix the bug" --prefer-cheap
noryx run --prompt "review authentication" --quality maximum
noryx run --prompt "fix failing checks" --mode ci --output json
...
noryx intelligence inspect "Fix calculator.py without changing README.md" --strict --json

# Fast artifact and host smoke check
noryx deploy check --mode review --json

# Execute the real installed repair/adversarial gate and qualify only the supervised deployment scope
noryx deploy check --mode review --deep --output deploy-readiness.json --json

# Execute one real offline repair plus four adversarial truth-integrity scenarios
noryx benchmark offline-reliability --output offline-reliability.json

# Enforce a matched direct-versus-Noryx evaluation contract
noryx benchmark compare-matched --direct direct.json --noryx noryx.json

# Run a blind matched-repository Noryx-versus-Claude duel manifest
noryx benchmark duel --manifest competitive.json --output duel-report.json

# Produce behavioral sandbox evidence for this exact host
noryx sandbox qualify --workspace . --output sandbox-qualification.json
...
noryx benchmark duel --manifest private-superiority.json --output unsigned-report.json
...
noryx benchmark superiority-gate --report signed-report.json \
  --trust-policy /secure/operator/trust-policy.json
```

An optional currency ceiling requires explicit prices so Noryx never invents
provider pricing:

```bash
noryx --max-cost-usd 1.00 \
...
The default permission mode does not apply file edits immediately. Noryx
...
Noryx relies on kernel-level sandboxing for autonomous workflows (like the `autonomous` mode and CI presets) to enforce strict network and file-system boundaries for generated code execution:
...
Noryx uses kernel-level sandboxing for autonomous and editing workflows. Run
`noryx --doctor` to check your sandbox status. Without a native backend, modes
...
needed. The `noryx --doctor` command will confirm it is operational.
...
can change, so `noryx --list-models` is the runtime source of truth.

| Noryx key | Provider model | Role |
|---|---|---|
...
If NVIDIA is unavailable and `GROQ_API_KEY` is configured, Noryx uses Groq
...
Noryx does not treat an assistant's statement as proof of completion. It
...
noryx benchmark --manifest benchmarks/core.json \
  --output benchmarks/results/noryx-3.4.0.json

# Validate the large single-prompt product contract without spending credits
noryx benchmark --manifest benchmarks/long_horizon.json --dry-run
...
Every turn is saved under the Noryx state directory with `request.json`,
...
```bash
noryx runs
noryx inspect <run-id>
noryx replay <run-id>
noryx resume <run-id>
noryx rollback <run-id>
```
...
- **Windows Support**: Windows does not have a native kernel sandbox isolation implementation in Noryx.

## Built-in tools

Noryx exposes 38 built-in tools across file editing, repository intelligence,
...
```bash
.venv/bin/ruff check noryx tests
.venv/bin/pytest
