...
SECRET_PATTERNS = [
    # API Keys & Tokens
    (
        r"(?i)(api[_-]?key|secret[_-]?key|auth[_-]?token|access[_-]?token)\s*[:=]\s*['\"]?([A-Za-z0-9_\-.~+]{12,})['\"]?",
        "SECRET_KEY_REDACTED",
    ),
    (r"(ghp_[A-Za-z0-9]{36}|github_pat_[A-Za-z0-9]{22}_[A-Za-z0-9]{59})", "GITHUB_TOKEN_REDACTED"),
    (r"(AKIA[0-9A-Z]{16})", "AWS_ACCESS_KEY_REDACTED"),
    (
        r"-----BEGIN (RSA|EC|OPENSSH|DSA|PRIVATE) KEY-----[\s\S]*?-----END \1 KEY-----",
...
class SecretProtector:
    ...
    def sanitize(content: str, file_path: str = "") -> tuple[str, bool]:
        ...
        if SecretProtector.is_secret_file(file_path):
            return (
                f"[PROTECTED FILE: {file_path} - Secret values redacted by security policy]",
                True,
            )
